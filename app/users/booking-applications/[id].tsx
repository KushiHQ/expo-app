...
// ─── local UI primitives (same pattern as hosting form step-8) ────────────────
      ...
      <DetailsLayout
        ...
        footer={
          <View className="flex-row gap-4 p-4 pb-8" style={{ backgroundColor: colors.background }}>
            {app?.booking?.id && (
              <Button type="primary" className="flex-1" onPress={() => router.push(`/bookings/${app.booking?.id}`)}>
            ...
            {canCancel && (
              ...
              >
                <ThemedText style={{ color: hexToRgba(colors.error, 0.9) }}>Cancel Application</ThemedText>
              </Button>
            )}
          </View>
        }
        ...
        {fetching ? (
          ...
          <View style={{ gap: 14, paddingBottom: 8 }}>
            ...
            <ReviewSection icon={<ShieldCheck size={15} color={colors.primary} />} title="Application Status">
              ...
              <DataRow
                label="Last Updated"
                value={app?.lastUpdated ? new Date(app.lastUpdated).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' }) : '—'}
              />
            ...
            {app?.commencementDate && (
              <ReviewSection icon={<CalendarDays size={15} color={colors.primary} />} title="Lease Terms">
                <DataRow
                  label="Commencement"
                  value={new Date(app.commencementDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}
                  accent
                />
            ...
            <ReviewSection icon={<Briefcase size={15} color={colors.primary} />} title="Guest Profile">
              ...
              {app?.guestFormData?.guarantorRelationships && (
                <DataRow label="Guarantor" value={toTitleCase(app.guestFormData.guarantorRelationships)} />
              )}
            </ReviewSection>
