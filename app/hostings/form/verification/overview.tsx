...
export default function VerificationOverview() {
	...
	const handleRequestAgain = (tier: HostingVerificationTier) => {
		router.push(
			`/hostings/form/verification/request?id=${hostingIdStr}&tier=${tier}`,
		);
	};
	...
	if (isTopTier) {
		return (
			...
			>
				...
				>
					Your hosting displays the Kushi Vetted badge. Guests see a
					fully-verified property and rank it higher in search.
				</ThemedText>
			</View>
		);
	}
			...
			{next ? (
				...
				>
					...
					>
						...
						{!inFlight ? (
							<ArrowRight size={14} color={colors["primary-content"]} />
						) : null}
					</Button>
				</View>
			) : null}
...
const EmptyHistory = () => {
	...
	return (
		...
		>
			...
			>
				When you submit your first request, you'll see it tracked here with
				status updates, admin feedback, and the documents you uploaded.
			</ThemedText>
		</View>
	);
};
