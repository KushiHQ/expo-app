...
const HostingMapCard: React.FC<Props> = ({ hosting, onClose }) => {
	...
	const { id, title, city, state, price, averageRating, totalRatings, coverImage, paymentInterval } = hosting;

	return (
		<Animated.View
			entering={FadeInDown.springify().damping(20).stiffness(150)}
			exiting={FadeOutDown}
			className="absolute bottom-10 left-5 right-5 rounded-[24px] overflow-hidden shadow-xl"
			style={{
