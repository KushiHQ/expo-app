...
import {
	generateRNFile,
	formMutation,
} from "@/lib/services/graphql/utils/fetch";
import {
...
const RequestVerificationForm: React.FC<Props> = ({
	...
	const allPicked = useMemo(() => {
		if (tierData?.hostingVerificationTier?.documentRequirements.length === 0)
			return true;
		return tierData?.hostingVerificationTier?.documentRequirements.every(
			(doc) => Boolean(picked[doc.title]),
		);
	}, [picked]);
	...
	const handleSubmit = async () => {
		...
		try {
			const names =
				tierData?.hostingVerificationTier?.documentRequirements.map(
					(v) => v.title,
				) ?? [];
			const files = (
		...
		<View className="gap-5">
			{title ? (
				<View className="gap-1">
					...
					>
						Upload the documents Kushi admins will review. Accepted: PDF or
						image.
					</ThemedText>
				</View>
			) : null}
				...
				<View>
					<View className="mb-3 flex-row items-center justify-between">
						...
						{(tierData?.hostingVerificationTier?.documentRequirements ?? [])
							...
							>
								{
									tierData?.hostingVerificationTier?.documentRequirements.filter(
										(d) => Boolean(picked[d.title]),
									).length
								}
								/
								{tierData?.hostingVerificationTier?.documentRequirements.length}{" "}
								uploaded
							</ThemedText>
						) : null}
					</View>
