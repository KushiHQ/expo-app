  ...
  ({ text, providedValues, replace = true, hosting, application }) => {
    ...
    const replacedText = React.useMemo(() => {
      ...
      if (hosting) {
        ...
        // Property
        variablesMap["PROPERTY_DESCRIPTION"] = capitalize(
          hosting.title ?? "",
          true,
        );
        ...
        variablesMap["MEDIATION_INSTITUTION"] = mediationInstitutionLabel(
          hosting.state,
        );
        variablesMap["COURT_JURISDICTION_PHRASE"] = courtJurisdictionPhrase(
          hosting.state,
        );

        // Room counts
        const roomCount = (name: string) => {
          const room = hosting.rooms?.find(
            (r) => r.name?.toLowerCase() === name.toLowerCase(),
          );
          return room?.count != null ? String(room.count) : "—";
        };
        variablesMap["NUMBER_OF_BEDROOMS"] = roomCount(
          keyOf(Room, Room.Bedroom) ?? "",
        );
        variablesMap["NUMBER_OF_BATHROOMS"] = roomCount(
          keyOf(Room, Room.Bathroom) ?? "",
        );
        variablesMap["NUMBER_OF_LIVING_ROOMS"] = roomCount(
          keyOf(Room, Room.LivingRoom) ?? "",
        );
        ...
        if (hosting.paymentDetails) {
          variablesMap["BANK_ACCOUNT_NAME"] =
            hosting.paymentDetails.accountName ?? "N/A";
          variablesMap["BANK_NAME"] =
            hosting.paymentDetails.bankDetails?.name ??
            hosting.paymentDetails.bankCode ??
            "N/A";
          variablesMap["BANK_ACCOUNT_NUMBER"] =
            hosting.paymentDetails.accountNumber ?? "N/A";
        }
        ...
        if (hosting.verification) {
          ...
          variablesMap["VERIFICATION_TIER"] = verificationTierLabel(
            v.verificationTier,
          );
          variablesMap["TITLE_TYPE"] =
            v.titleType ?? "Certificate of Occupancy";
        ...
        if (hosting.host?.user) {
          ...
          if (hu.kyc) {
            variablesMap["LANDLORD_ID_TYPE"] =
              hu.kyc.idDocumentType ?? "Government-issued ID";
            variablesMap["KYC_REFERENCE_ID"] =
              hu.kyc.youverifyReferenceId ?? "Pending";
          }
        }
      }
      ...
      if (application) {
        ...
        variablesMap["TENANT_ADDRESS"] = capitalize(
        ...
        const duration = hostingDuration(
          ...
          application.commencementDate
            ? new Date(application.commencementDate)
            : undefined,
        );
