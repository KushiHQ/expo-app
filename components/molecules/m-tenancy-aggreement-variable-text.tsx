  ...
  ({ text, providedValues, replace = true, hosting, application, tenantUser, fees }) => {
    ...
    const replacedText = React.useMemo(() => {
      ...
      if (hosting) {
        ...
        variablesMap['NUMBER_OF_BEDROOMS'] = roomCount(keyOf(Room, Room.Bedroom) ?? '');
        variablesMap['NUMBER_OF_BATHROOMS'] = roomCount(keyOf(Room, Room.Bathroom) ?? '');
        variablesMap['NUMBER_OF_LIVING_ROOMS'] = roomCount(keyOf(Room, Room.LivingRoom) ?? '');
