...
/**
 * Staff-entered lead (source = MANUAL) — for backfilling leads that arrived
 * by email/phone/field before or outside the web form.
 */
export type AdminCreateHostLeadInput = {
  ...
  status?: InputMaybe<HostLeadStatus>;
};
...
/** Admin view of a landlord onboarding lead (from /hosts or field sources). */
export type AdminHostLead = {
  __typename?: 'AdminHostLead';
...
export type AdminHostLeadFilter = {
  /** Substring match against name / phone / area. */
  search?: InputMaybe<Scalars['String']['input']>;
  status?: InputMaybe<HostLeadStatus>;
};

export type AdminHostLeadResponse = {
  __typename?: 'AdminHostLeadResponse';
  data?: Maybe<AdminHostLead>;
  message: Scalars['String']['output'];
};
...
export type AdminUpdateHostLeadInput = {
  adminNotes?: InputMaybe<Scalars['String']['input']>;
...
/**
 * Public input from the /hosts landing page lead form — no auth required, so
 * the resolver validates and length-caps every field.
 */
export type CreateHostLeadInput = {
  /** Where the property is (e.g. "Karu", "Mararaba"). */
  area: Scalars['String']['input'];
  name: Scalars['String']['input'];
  note?: InputMaybe<Scalars['String']['input']>;
  phone: Scalars['String']['input'];
...
/** How the property's electricity is billed — a key renter concern in Nigeria. */
export enum ElectricityBilling {
  Estimated = 'ESTIMATED',
  NoSupply = 'NO_SUPPLY',
  PostpaidMeter = 'POSTPAID_METER',
  PrepaidMeter = 'PREPAID_METER'
}
...
/**
 * Pipeline status for a landlord onboarding lead. Stored as TEXT on the row;
 * unknown values fall back to `New` so a bad write can't break reads.
 */
export enum HostLeadStatus {
  Closed = 'CLOSED',
  Contacted = 'CONTACTED',
  New = 'NEW',
  Onboarded = 'ONBOARDED'
...
export type Hosting = {
  ...
  createdAt: Scalars['String']['output'];
  description?: Maybe<Scalars['String']['output']>;
  electricityBalanceCleared?: Maybe<Scalars['Boolean']['output']>;
  /** Electricity disclosure (null when not applicable, e.g. land). */
  electricityBilling?: Maybe<ElectricityBilling>;
  ...
  /**
   * Whether this hosting can be transacted directly.
   * - Standalone: yes.
   * - Parent: only when it's a Group sale (the whole property is the
   * product); an Individual parent is a container — you book a unit.
   * - Child: yes, unless its parent is a Group sale, in which case the unit
   * is read-only showcase and only the parent is transactable.
   */
...
export type HostingInput = {
  ...
  country?: InputMaybe<Scalars['String']['input']>;
  description?: InputMaybe<Scalars['String']['input']>;
  electricityBalanceCleared?: InputMaybe<Scalars['Boolean']['input']>;
  /**
   * Electricity disclosure (residential/commercial). Billing type + a
   * host-declared outstanding balance (shown only when not prepaid).
   */
  electricityBilling?: InputMaybe<ElectricityBilling>;
  electricityOutstandingBalance?: InputMaybe<Scalars['Decimal']['input']>;
  facilities?: InputMaybe<Array<Scalars['String']['input']>>;
  id?: InputMaybe<Scalars['String']['input']>;
  ...
  /**
   * Who manages the listing. Defaults to Kushi-Managed server-side when
   * omitted; Agent-Managed turns it into an advertise-only listing.
   */
  managementType?: InputMaybe<ManagementType>;
  ...
  title?: InputMaybe<Scalars['String']['input']>;
  /**
   * Meaningful only on a parent: whether its units are sold individually
   * (default) or the whole property is sold as one group.
   */
  unitStructure?: InputMaybe<UnitStructure>;
};
...
/**
 * Who transacts the property: Kushi (full on-platform flow) or an agent
 * (advertise-only; users contact the agent, deal happens off-platform).
 */
export enum ManagementType {
  AgentManaged = 'AGENT_MANAGED',
  KushiManaged = 'KUSHI_MANAGED'
}
...
export type Mutations = {
  ...
  /**
   * Staff: manually add a lead (source = MANUAL) — for backfilling leads
   * that arrived by email/phone/field before or outside the web form.
   */
  adminCreateHostLead: AdminHostLeadResponse;
  ...
  /** Staff: move a lead through the pipeline (status) and/or set notes. */
  adminUpdateHostLead: AdminHostLeadResponse;
   ...
   */
  createHostLead: MessageResponse;
...
export type MutationsAdminCreateHostLeadArgs = {
  input: AdminCreateHostLeadInput;
};
...
export type MutationsAdminUpdateHostLeadArgs = {
  input: AdminUpdateHostLeadInput;
};
...
export type MutationsCreateHostLeadArgs = {
  input: CreateHostLeadInput;
};
...
export type Query = {
  ...
  /** Landlord onboarding leads, newest first. */
  adminHostLeads: Array<AdminHostLead>;
  adminHostLeadsCount: Scalars['Int']['output'];
...
export type QueryAdminHostLeadsArgs = {
  filter?: InputMaybe<AdminHostLeadFilter>;
  pagination?: InputMaybe<PaginationInput>;
};


export type QueryAdminHostLeadsCountArgs = {
  filter?: InputMaybe<AdminHostLeadFilter>;
};
...
export type CreateHostingRoomImageMutation = { __typename?: 'Mutations', createHostingRoomImage: { __typename?: 'HostingRoomImageResponse', message: string, data?: { __typename?: 'HostingRoomImage', id: string, asset: { __typename?: 'Asset', id: string, publicUrl: string, lastUpdated: string } } | null } };
